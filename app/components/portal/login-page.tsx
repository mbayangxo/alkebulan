    ...
    >
      <div className="w-full max-w-sm">
        ...
        <form action={signInWithGoogle}>
          <button
            type="submit"
            className="w-full flex items-center justify-center gap-3 py-3.5 rounded-2xl text-sm font-semibold transition-all hover:bg-gray-50 active:scale-[0.98]"
            style={{
              background: "white",
        ...
        {showSms && (
          <div className="mt-4 flex flex-col gap-3">
            {smsStep === "idle" && (
              <>
                ...
                <button
                  ...
                  className="w-full py-3.5 rounded-full text-white font-bold text-sm hover:brightness-110 active:scale-[0.98] transition-all disabled:opacity-50"
              ...
              <>
                <div>
                  ...
                  <input
                    ...
                    placeholder="123456"
                    maxLength={6}
                    className="w-full bg-white rounded-2xl px-4 py-3.5 text-sm outline-none border-2 border-transparent transition-colors tracking-widest text-center"
                    style={{
                      color: "#111111",
                      boxShadow: "0 1px 8px rgba(0,0,0,0.06)",
                    }}
                ...
                <button
                  ...
                  disabled={smsLoading || otp.length < 6}
                  className="w-full py-3.5 rounded-full text-white font-bold text-sm hover:brightness-110 active:scale-[0.98] transition-all disabled:opacity-50"
                  style={{ background: "#FF1F7D" }}
                >
                ...
                <p className="text-center text-xs text-gray-400">
                  ...
                  <button
                    type="button"
                    onClick={() => { setSmsStep("idle"); setOtp(""); setSmsError(null); }}
                    className="underline"
                    style={{ color: "#FF1F7D" }}
                  >
                    Resend
            ...
            {smsError && (
              <div className="rounded-2xl p-3 text-sm text-center" style={{ background: "#FFF0F5", color: "#FF1F7D" }}>
                {smsError}
              </div>
            )}
          </div>
