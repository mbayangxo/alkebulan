...
import { MemberPhotoVerification } from "@/app/components/admin/member-photo-verification";
...
export default async function AdminVerificationPage() {
  ...
  return (
    <FounderShell title="Verification" subtitle="Working moderation queue">
      {loadError ? (
        <p className="bb-admin-login-error">{loadError}</p>
      ) : (
        <>
          <VerificationQueue initialRows={rows} />
          <MemberPhotoVerification />
        </>
      )}
    </FounderShell>
  );
